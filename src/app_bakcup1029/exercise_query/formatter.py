    ...
    for exercise in exercises:
        ...
        if ex_type == "LISTEN_IMAGE_TRUE_FALSE":
            ...
            if ex_type in ["LISTEN_SENTENCE_QA","LISTEN_DIALOGUE_QA","LISTEN_PARAGRAPH_QA"]:
                ...
                correct_answer_map = metadata.get("correctAnswer", {})
                try:
                    content = schemas.ReadWordGapFillContent(
                        prompt=content_data.get("prompt", exercise.prompt or "请根据语境，选择最合适的词填空。"),
                        text=content_data.get("text", ""),
                        blanks=[schemas.GapFillBlankItem(**b) for b in content_data.get("blanks", [])]
                    )
                except Exception as e:
                    print(f"[Formatter_Warning] 跳过 READ_WORD_GAP_FILL (ID: {exercise.id})， metadata 格式无效: {e}")
                    continue
                formatted_list.append(
                    schemas.ReadWordGapFillExercise(
                        ...
                        exerciseType=ex_type,
                        content=content,
                        correctAnswer=correct_answer_map,
                    )
                )
        ...
        elif ex_type == "TRANSLATE_WORD_ORDER":
            meta_content = metadata.get("content", {})
            
            words_list = meta_content.get("words", [])
            items = [schemas.WordOrderItem(**item) for item in words_list]
            
            content = schemas.TranslateWordOrderContent(

                prompt=meta_content.get("prompt", exercise.prompt or ""),
                originalSentence=meta_content.get("originalSentence", ""),
                items=items 
            )
            formatted_list.append(
                schemas.TranslateWordOrderExercise(
                    exerciseId=str(exercise.id),
                    exerciseType=ex_type,
                    content=content,
        ...
        elif ex_type == "SPEAK_FOLLOW":
            content = schemas.SpeakFollowContent(
                prompt=exercise.prompt or "请跟着示例音频朗读下列句子。",
                sentence=metadata.get("sentence", ""),
                pinyin=metadata.get("pinyin", ""),
                sampleAudioUrl=media_map.get("sample_audio") or ""
            )
            formatted_list.append(
                schemas.SpeakFollowExercise(
                    exerciseId=str(exercise.id),
                    exerciseType=ex_type,
                    content=content,
                )
            )
        elif ex_type == "STROKE_ORDER_WRITING":
            
            metadata = exercise.meta or {}

            animation_url = build_public_url(
                metadata.get("animation"), 
                base_url
            )

            content = schemas.StrokeOrderWritingContent(
                prompt=exercise.prompt or "请在写字板上写出这个字。",
            ...
            formatted_list.append(
                schemas.StrokeOrderWritingExercise(
                    exerciseId=str(exercise.id),
                    exerciseType=ex_type,
                    content=content
                )
