...
class MatchReq(BaseModel):
    """听力·看图配对 —— 请求体"""
    keyword: str               # 至少 2~6 个关键词（每个关键词生成一条音频 + 一张图片）
    hskLevel: int = Field(..., ge=1, le=6)
    lang: str = "zh-CN"
    difficulty: int = Field(2, ge=1, le=5)
    seed: Optional[int] = None        # 选填：用于固定打乱顺序的随机种子（可复现）
...
class ReadImageMatchReq(BaseModel):
    ...
    hskLevel: int = Field(..., ge=1, le=6)
    difficulty: int = Field(2, ge=1, le=5)
    seed: Optional[int] = None          # 控制乱序和配对数量的随机种子
...
class GapFillContent(BaseModel):
    """定义多项填空题的 'content' 结构"""
    prompt: str
    text: str  # 包含 ___ 的句子
...
class ReadingDialogMatchResp(BaseModel):
    ...
    hsk_level: int
    difficulty: int
    shuffled_questions: Dict[str, Dict[str, str]]   #
    shuffled_answers: Dict[str, Dict[str, str]]     #
...
class ReadingGapFillResp(BaseModel):
    ...
    exerciseId: str
    exerciseType: str = "READ_WORD_GAP_FILL"
...
class ReadParagraphComprReq(BaseModel):
    ...
    hskLevel: int = Field(..., ge=1, le=6)
    difficulty: int = Field(2, ge=1, le=5)
    textLength: int = Field(100, ge=50, le=400)   
    numQuestions: int = Field(3, ge=1, le=8)      
    seed: Optional[int] = None                  
...
class TranslateWordOrderResp(BaseModel):
    content: TranslateWordOrderContent
    correctAnswer: List[str]  

class SpeakAlongReq(BaseModel):
...
class SpeakAlongContent(BaseModel):
    prompt: str
    sentence: str
    pinyin: str
    sampleAudioUrl: str

class SpeakAlongResp(BaseModel):
    content: SpeakAlongContent
